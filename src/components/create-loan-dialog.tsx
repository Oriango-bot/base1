...
export default function CreateLoanDialog({ borrowerId, canApply = true }: CreateLoanDialogProps) {
  ...
  const { control, register, handleSubmit, formState: { errors }, trigger, getValues } = form;
  ...
  const nextStep = async () => {
    ...
    if (step === 3) fields = ['productType', 'amount', 'loanPurpose', 'repaymentSchedule', 'interestRate', 'processingFee'];
